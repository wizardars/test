...
namespace Test
{
    class Program
    {
        ...
        private static void TrimTest()
        {
            ...
            Stopwatch stopwatch = new Stopwatch();
            stopwatch.Start();
            var List = new List<ArraySegment<byte>>(s_iterations);
        ...
        private static void SliceTest()
        {
            ...
            try
            {
                ...
                Print($"Segment taken from region:{Array.FindIndex(s_arraySegmentPool.UnderlyingLayoutArray, PredicateFindOne)} DangerousRent().Slice(0, _segment_size)");
                ...
                Print($@"Segment taken from region:{Array.FindIndex(s_arraySegmentPool.UnderlyingLayoutArray, PredicateFindOne)} DangerousRent().Slice(_segment_size / 2, _segment_size / 2)");
                if (ReturnAndCheck(ref ArraySegment) == false)
                {
                    IsError = true;
                    return;
                ...
                Print($"Segment taken from region:{Array.FindIndex(s_arraySegmentPool.UnderlyingLayoutArray, PredicateFindOne)} DangerousRent().Slice(_segment_size, 0)");
                if (ReturnAndCheck(ref ArraySegment) == false)
                {
                    IsError = true;
                    return;
                ...
                //case 4
                ArraySegment = s_arraySegmentPool.DangerousRent(0);
                Print($"Segment taken from region:{Array.FindIndex(s_arraySegmentPool.UnderlyingLayoutArray, PredicateFindOne)} DangerousRent(0)");
                if (ReturnAndCheck(ref ArraySegment) == false)
                {
                    IsError = true;
                    return;
            ...
            {
                Print($"Elapsed:{stopwatch.ElapsedMilliseconds}ms. Capacity:{s_arraySegmentPool.Capacity} Count:{s_arraySegmentPool.Count} Fails:{s_arraySegmentPool.FailsCount}", ConsoleColor.Yellow);
                if (IsError == false & s_arraySegmentPool.Capacity == 10 & s_arraySegmentPool.Count == 0 & s_arraySegmentPool.FailsCount == 0)
                    Print("TEST PASSED", ConsoleColor.Green);
                else
                    Print("TEST FAILED", ConsoleColor.Red);
            }            
